...
import { BorderColor, Grade, UndoRounded } from "@mui/icons-material";
...
function TradeFeedOld() {
  ...
  //     if (firstSelectedRow) {
  //       setSelectedRegCodes(firstSelectedRow.reg_code);
  //     }
  //   } else {
  //     setSelectedRegCodes('');
  //   }
  ...
  const categorizeSelectedRows = () => {
    ...
    if (regCode === "CAMS") {
      ...
      setReload(true);
      axios
        .post(`${baseUrl}/download-cams-zip`, requestData, {
          headers: {
            ...headers,
      ...
      // console.log(selectedNomineeRegCode);
      axios
        .post(`${baseUrl}/download-nominee-karvy-zip`, requestDatas, {
          headers: {
            ...headers,
            "Content-Type": "application/json",
          },
  ...
  //     const firstSelectedRow = jsonData.payload.find((row) => row.id === selectedData[0]);
  ...
  //       disableRows = jsonData.payload.filter((row) => row.reg_code === 'KARVY');
  //     } else if (firstSelectedRegCode === 'KARVY') {
  //       // Disable rows with reg_code 'cams'
  //       disableRows = jsonData.payload.filter((row) => row.reg_code === 'CAMS');
  //     }
  //   }
  ...
  const checkSelection = (selectedData) => {
    ...
    if (selectedRows.length > 0) {
      ...
      const firstSelectedRowAC = selectedRows[0];

      const firstSelectedAmcCode = firstSelectedRowAC ? firstSelectedRowAC.amc_code : null;

      // Calculate the sum of 'nom_tiff_kb_size' for selected rows
      const sumNomTiffSize = selectedRows.reduce((sum, row) => {
        if (row.nom_tiff_kb_size !== 'N/A') {
          console.log("nom size: ", parseFloat(row.nom_tiff_kb_size))
          return sum + parseFloat(row.nom_tiff_kb_size);
        }
        return sum;
      }, 0);

      // Check if the sum is greater than or equal to 4 MB (4000 KB)
      console.log("sum of nom tiff size", sumNomTiffSize)
      if (sumNomTiffSize >= 4000) {
        ...
        if (currentRow && currentRow.nom_tiff_kb_size !== 'N/A') {
      ...
      jsonData.payload.forEach((row) => {
        ...
        if (row.amc_code !== firstSelectedAmcCode && row.reg_code === 'KARVY' && !selectedData.includes(row.id)) {
  ...
  // const tifffilesize=(selecteddata)=>
  // {
  //   let size=[]
  //   if(selecteddata.length>0)
  //   {
  //     const select = jsonData.payload.find((row) => row.id === selecteddata[0]);
  //     const sizeSelect=select?select.nom_tiff_size:null;
  ...
  const columns = [
    ...
    //   renderCell: (params) => (
    //     <Checkbox
    //     disabled={selectedRegCodes !== params.row.reg_code}
    ...
    },
    {
      ...
      //     {...params}
      //     disabled={
      //       (selectedRegCode === "CAMS" && params.row.reg_code === "KARVY") ||
      //       (selectedRegCode === "KARVY" && params.row.reg_code === "CAMS")
      //     }
      //   />
      // ),
    },
    ...
    },
    {
      ...
      renderCell: (params) => {
        const rowId = params?.row?.id;
        const isFileUploaded = rowId !== undefined ? green[rowId] : false;
        const nom_status = params.row.nom_status;
        console.log("nom_status: ", nom_status);
        return (
          <>
            <div className="uploadBtn">
              <Button
                ...
                size="small"
                sx={{
                  backgroundColor: nom_status === "generated" ? "green" : "#242056",
                  // color: isFileUploaded ? "white" : "inherit",
                }}
              >
                {nom_status}
  ...
  const handleFileChange = (event) => {
    ...
    if (selectedRowId !== null) {
      ...
      if (selectedFile?.type !== "application/pdf" && selectedFile?.type !== "image/tiff") {
  ...
  useEffect(() => {
    const getPoaList = async () => {
      const res = await fetch(`${baseUrl}/list-rta-nom-old?page=1&per_page=${limit}`);
  ...
  const fetchPoaList = async (currentPage) => {
    ...
    const res = await fetch(
      `${baseUrl}/list-rta?page=${currentPage}&per_page=${limit}`
        ...
        >
          ...
          >
            ...
            <DataGrid
              ...
              //     setSelectedRegCodes(jsonData.payload[newSelection[0]].reg_code);
              //   } else {
              //     setSelectedRegCodes('');
              //   }
              //   console.log(newSelection);
              // }}
              // isRowSelectable={(params)=>params.row.reg_code==selectedRegCodes}
              onRowSelectionModelChange={(newSelection) => {
